...
export function RequestBuilder() {
  ...
  const [selectedCollectionForSave, setSelectedCollectionForSave] =
    useState('');
    ...
    try {
      if (isLocalhostUrl(url)) {
        ...
        const first = await fetch(url, {
          ...
          body:
            ['POST', 'PUT', 'PATCH', 'DELETE'].includes(method) && body
              ? JSON.stringify(safeParseJSON(body) ?? body)
              : undefined,
        });
        let r = first;
        ...
        const data = ct.includes('application/json')
          ? await r.json().catch(async () => await r.text())
          : await r.text();
  ...
  async function onSave() {
    ...
    try {
      if (!response || response.status == null)
        throw new Error('Send a request first');
      const payload = {
        method,
        url,
        headers: builtHeaders,
          ...
          <Card className="border border-border/60 bg-background/60 shadow-sm">
            ...
            <CardContent className="space-y-3">
              {headers.map((h) => (
                <div key={h.id} className="flex gap-2 items-center">
                  <div className="flex-1">
                    <Label htmlFor={`header-key-${h.id}`} className="sr-only">
                      Header Name
                    </Label>
                    <Input
                      id={`header-key-${h.id}`}
                  ...
                  <div className="flex-1">
                    <Label htmlFor={`header-value-${h.id}`} className="sr-only">
                      Header Value
                    </Label>
                    <Input
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    className="h-9 w-9 hover:bg-red-100/60 hover:text-red-600 dark:hover:bg-red-950/30"
                  >
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                className="w-full border border-dashed border-border/60 hover:bg-muted/60"
              >
